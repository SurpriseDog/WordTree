...
def open_any(filename):
	"Return file handle for variety of compression types."

	if not os.path.exists(filename):
		raise ValueError("Filename does exist: " + filename)
	...
	if ext == '.bz2':
		...
		eprint("Filename not supported:", filename)
		raise ValueError("Only frequency files in the format: .gz .bz2 or .txt are supported.")
